...
# aiofiles and httpx are both async compatible libraries for file IO and HTTP requests respectively.
# use a semaphore to ensure only a finite number of concurrent tasks are initiated
# This avoids the local machine or host server from being overloaded with multiple simultaneous requests or tasks.
# The image processing function is a CPU bound task, so multiprocessing is the only way to speed this up.
# To avoid stressing the system, a max count of CPU workers can be established.

# With these optimisations, the code above was executed 4x faster.